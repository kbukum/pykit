...
description: >-
    ...
    pykit's engineering baseline, validate the affected packages, and mark the step done. Use to
...
Apply the current step's actions **test-first**, honoring pykit's engineering baseline — the plan
...
- **Canonical reuse.** Reuse or extend the owning core package / stdlib before adding new code;
...
- **Typed & minimal.** No broad `Any` on public surfaces (documented opaque exceptions only);
  PEP 695 generics and Protocols; typed `AppError` preserving cause via `raise ... from e`;
  timeout + cancellation on remote `await`s.
- **Root-cause, no shims.** Redesign cleanly; remove the old path completely (pre-stable, no
  back-compat).
- **Readable files & injected composition.** Split by concern into focused modules; injected
...
- **Validate** the affected packages with the `validate` skill (make, scoped): `make lint`,
  `make typecheck`, `make test` green; async tests deterministic and parallel-safe. A step does
  not land red.
