...
description: >-
    ...
    that can be applied iteratively. Every plan is bound to pykit's engineering baseline. Use
    when scoping a feature, refactor, parity port, or release, or when asked to plan or break
    down work.
user-invocable: true
---

# Planning pykit work as applyable step files
...
kebab-case (`tmp/component-lifecycle-drain/`, `tmp/storage-s3-multipart/`) — the same high-level
...
- **`NN-topic.md`** step files (when the work is multi-step) — zero-padded and ordered by
  ...
  files — split into step files as soon as the work is iterative or spans packages.
...
## Files touched
- `core/packages/pykit-x/**`, new `contrib/pykit-y/**`, ...

## Acceptance criteria
- [ ] Behavior written test-first; deterministic and parallel-safe on affected packages.
...
A plan may **not** invent a lighter standard than pykit's. Its cross-cutting rules restate — and
...
- **Canonical reuse.** Reuse or enhance the owning core package / stdlib before writing new code;
  never duplicate a shared concern.
- **Typed & minimal APIs.** No broad `Any`/untyped `object` on public surfaces (documented opaque
  exceptions only); PEP 695 generics and Protocols; typed `AppError` preserving cause via
  `raise ... from e`; timeouts + cancellation on every remote `await`.
- **Root-cause, no shims.** Pre-stable: redesign cleanly and remove the old path; no compat
  shims or half-migrations.
- **Readable files.** Split by concern into focused modules — never pile into one file.
