...
A plan folder under `tmp/` — e.g. `tmp/storage-s3-multipart/`. If the caller does not name one,
...
- **Validate the affected packages** with the `validate` skill (make, scoped) — do not proceed
...
Every step is executed against pykit's engineering baseline, not a looser plan-local standard.
