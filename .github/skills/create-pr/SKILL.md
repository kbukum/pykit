...
description: >-
    ...
    pykit's engineering baseline. Use only when explicitly asked to create or open a PR.
user-invocable: true
---
...
simplified**, and whose template sections are filled **honestly** against pykit's baseline.
...
Answer, in your head: what capability/fix/refactor is this, which packages it touches, what a
reviewer must understand to judge it, and whether it changes a public abstraction (error codes,
`Component` lifecycle, `Provider`/stream shapes, pipeline operators) that the sibling kits
(rskit/gokit) mirror.
...
- **Type of Change / Package(s) Affected** — mark accurately (package names, e.g. `pykit-storage`).
- **Changes Made** — a short, grouped bullet list of the *key* changes by concern (e.g. "typed
  ...
  log.
- **Testing** — check only the gates you actually ran; scope to affected packages. These map to
...
- **Checklist** — tick only what is genuinely true (including `uv.lock` committed if deps changed,
  ...
  signal; a falsely checked one wastes reviewer trust. Do not narrate prior bugs or how they were
  fixed — describe the change as it stands.
...
The PR asserts the change meets pykit's engineering baseline
