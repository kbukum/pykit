...
from pykit_llm_anthropic import AnthropicConfig, AnthropicProvider
...
class TestAnthropicStream:
    ...
    async def test_stream_tool_use(self, config):
        """Tool use deltas in the SSE stream assemble into canonical tool blocks."""
        lines: list[str] = []
        ...
        try:
            ...
            assert _assemble_tool_calls(chunks) == [
                ToolUseBlock(id="toolu_123", name="get_weather", input={"location": "NYC"})
            ]
...
class TestAnthropicResponseToolCalls:
    @pytest.fixture
    def config(self):
        return AnthropicConfig(api_key="sk-ant-test", model="claude-sonnet-4-20250514")

    async def test_complete_parses_tool_use_block(self, config):
        data = {
            "id": "msg-test",
            "type": "message",
            "role": "assistant",
