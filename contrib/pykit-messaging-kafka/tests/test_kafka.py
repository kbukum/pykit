...
class TestProducer:
    async def test_start_stop(self) -> None:
        with patch("pykit_messaging_kafka.producer.AIOKafkaProducer") as MockProducer:
...
class TestConsumer:
    async def test_start_stop(self) -> None:
        with patch("pykit_messaging_kafka.consumer.AIOKafkaConsumer") as MockConsumer:
            mock_instance = AsyncMock()
...
class TestComponent:
    async def test_lifecycle(self) -> None:
        with (
            patch("pykit_messaging_kafka.component.KafkaProducer") as MockProducer,
            patch("pykit_messaging_kafka.component.KafkaConsumer") as MockConsumer,
        ):
            mock_prod = AsyncMock()
            mock_cons = AsyncMock()
