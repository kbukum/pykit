...
class WorkerPool:
    ...
    def __init__(self, config: PoolConfig | None = None) -> None:
        ...
        self._slot_semaphores = [asyncio.Semaphore(1) for _ in range(max(self._config.max_workers, 1))]
    ...
    def _drop_oldest_pending_locked(self) -> bool:
        while self._pending_order:
            ...
            entry = self._tasks.get(task_id)
            if entry is None or entry.task.status != TaskStatus.PENDING or entry.future is None:
                continue
            entry.task.status = TaskStatus.CANCELLED
