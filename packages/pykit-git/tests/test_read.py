...
def test_log_since_and_until(tmp_repo: Path) -> None:
    ...
    since_commits = repo.log(LogOptions(since=older_commit.committer.when + timedelta(hours=12), path_filter='beta.txt'))
...
def commit_with_date(repo_dir: Path, path: str, content: str, message: str, when: datetime) -> None:
    ...
    subprocess.run(['git', 'commit', '-m', message], cwd=repo_dir, check=True, capture_output=True, text=True, env=env)
