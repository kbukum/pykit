...
async def rotate(
    store: Store, old_key_id: str, cfg: RotationConfig | None = None
) -> RotationResult:
    ...
    grace_ends_at = datetime.now(timezone.utc) + cfg.grace_period

    await store.set_grace_period(old_key_id, grace_ends_at, "")
    return RotationResult(
        new_key=new_result, old_key_id=old_key_id, grace_ends_at=grace_ends_at
    )
