"""Extended tests for pykit-redis: TypedStore complex types, TTL edge cases,
...
class TestTypedStoreComplexTypes:
    ...
    async def test_nested_dict_round_trip(self, client: RedisClient) -> None:
    ...
    async def test_dataclass_round_trip(self, client: RedisClient) -> None:
    ...
    async def test_none_nested_field(self, client: RedisClient) -> None:
    ...
    async def test_empty_collections(self, client: RedisClient) -> None:
...
class TestTTLEdgeCases:
    ...
    async def test_ttl_zero_rejected_by_redis(self, client: RedisClient) -> None:
    ...
    async def test_ttl_positive(self, client: RedisClient) -> None:
        store: TypedStore[dict] = TypedStore(client, key_prefix="ttl")
        await store.save("k1", {"v": 1}, ttl=300)
        got = await store.load("k1")
        assert got is not None

    async def test_ttl_very_large(self, client: RedisClient) -> None:
    ...
    async def test_client_set_with_expiry(self, client: RedisClient) -> None:
...
class TestKeyPrefixIsolation:
    ...
    async def test_different_prefixes_isolated(self, client: RedisClient) -> None:
    ...
    async def test_prefix_stored_correctly(self, client: RedisClient) -> None:
    ...
    async def test_no_prefix_stored_bare(self, client: RedisClient) -> None:
        store: TypedStore[dict] = TypedStore(client)
        await store.save("bare-key", {"v": 1})
        raw = await client.get("bare-key")
        assert raw is not None

    async def test_delete_only_affects_own_prefix(self, client: RedisClient) -> None:
        store_a: TypedStore[dict] = TypedStore(client, key_prefix="svcA")
        store_b: TypedStore[dict] = TypedStore(client, key_prefix="svcB")
    ...
    async def test_full_key_method(self, client: RedisClient) -> None:
        store = TypedStore(client, key_prefix="pfx")
        assert store._full_key("k1") == "pfx:k1"

    async def test_full_key_no_prefix(self, client: RedisClient) -> None:
...
class TestConfigValidation:
    ...
    def test_decode_responses_false(self) -> None:
        cfg = RedisConfig(decode_responses=False)
...
class TestKeyInjection:
    ...
    async def test_special_character_keys(self, client: RedisClient) -> None:
    ...
    async def test_prefixed_key_correct(self, client: RedisClient) -> None:
    ...
    async def test_empty_key(self, client: RedisClient) -> None:
        store: TypedStore[dict] = TypedStore(client, key_prefix="safe")
...
class TestComponentExtended:
    ...
    async def test_name_custom(self) -> None:
        comp = RedisComponent(RedisConfig(name="session-cache"))
