"""Example: Database repository pattern and cache typed store.

Demonstrates:
- Async SQLite database with the Repository pattern
- TypedStore with key prefixes for cache
- Both wrapped in asyncio.run

NOTE: The database example uses an in-memory SQLite via aiosqlite.
NOTE: The cache example requires a running cache server (redis://localhost:6379).
"""
...
# ---------------------------------------------------------------------------
# 2. cache — TypedStore with key prefixes
# ---------------------------------------------------------------------------


async def demo_redis() -> None:
    """Show TypedStore usage with a cache backend.

    NOTE: requires a running cache server at localhost:6379.
    """
    from pykit_cache import CacheClient, CacheConfig, TypedStore

    print("\n=== cache TypedStore ===")
    print("  NOTE: requires running cache at localhost:6379")
    ...
    try:
        ...
        print(f"  cache unavailable (expected in demo): {type(exc).__name__}: {exc}")
    finally:
        await client.close()
